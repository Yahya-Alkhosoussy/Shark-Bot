...
def get_fish_result(username: str, user_id: int, size: str, fish_list: list[tuple[str, int]]) -> tuple[int, int, int]:
    ...
    for fish in fish_list:
        if fish[0] == "normal":
            ...
            legendary_fish += fish[1]
            add_fish_caught(username, user_id, size, "legendary")

    return normal_fish, shiny_fish, legendary_fish


@bot.tree.command(name="fish-multiple-times")
@discord.app_commands.describe(
...
async def link_twitch(interaction: discord.Interaction, twitch_name: str):
    ...
    if twitch_user_id is None:
        await interaction.response.send_message(f"Could not find someone with the twitch name {twitch_name}")
        return
    if sg.does_user_have_dex(interaction.user.id):
...
async def shark_game_tutorial(ctx: commands.Context):
    ...
    if not category:
        category = bot.get_channel(1446052298747805797)
    ...
    try:
        ...
        raise Exception(f"I got an error while putting permissions on myself. {str(e)}")
    ...
    if isinstance(ctx.author, discord.Member):
        try:
            ...
            raise Exception(f"Got an error while making the permissions. {str(e)} Debug statements: {debug}")
    else:
...
async def on_command_error(ctx: commands.Context, error):
    ...
    if not bot.shark_loops.is_idle:
        ...
        if isinstance(bot_channel, discord.TextChannel):
            ...
            if isinstance(error, commands.CommandNotFound):
                await bot_channel.send(f"{user} tried using command {ctx.message.content} and I do not recognize that")
            elif isinstance(error, commands.MissingPermissions):
